...
namespace ServerApiTemplate.Application.Features.V1.Roles.Services;
...
public static class PermissionMatrix
{
    ...
    public static async Task<IReadOnlyList<ActivityPermissionDto>> ToDtosAsync(
        IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, IEnumerable<CrudPermission> rows, CancellationToken ct)
    {
...
{
    public ActivityPermissionInputsValidator(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork)
    {
        RuleFor(x => x)
