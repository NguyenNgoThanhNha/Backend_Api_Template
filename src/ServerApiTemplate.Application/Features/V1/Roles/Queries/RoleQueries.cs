...
namespace ServerApiTemplate.Application.Features.V1.Roles.Queries;

public sealed record GetActivitiesQuery : IRequest<IReadOnlyList<ActivityDto>>;

public sealed class GetActivitiesQueryHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork)
...
public sealed class GetRolesQueryHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork) : IRequestHandler<GetRolesQuery, IReadOnlyList<RoleDto>>
{
...
public sealed class GetRoleDetailQueryHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork) : IRequestHandler<GetRoleDetailQuery, RoleDetailDto>
{
...
{
    public static async Task<RoleDetailDto> ReadAsync(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, Guid roleId, CancellationToken ct)
    {
