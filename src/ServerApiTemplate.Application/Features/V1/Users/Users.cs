namespace ServerApiTemplate.Application.Features.V1.Users.DTOs
{
    using ServerApiTemplate.Application.Features.V1.Roles.DTOs;
...
namespace ServerApiTemplate.Application.Features.V1.Users.Services
{
    using ServerApiTemplate.Application.Features.V1.Roles.DTOs;
    using ServerApiTemplate.Application.Features.V1.Roles.Services;
    using ServerApiTemplate.Application.Features.V1.Users.DTOs;
    ...
    public static class UserReader
    {
        ...
        public static async Task<UserListItemDto> GetAsync(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, Guid userId, CancellationToken ct) =>
        ...
        public static async Task<UserPermissionDetailDto> GetPermissionsAsync(
            IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, IPermissionService permissionService, Guid userId, CancellationToken ct)
        {
        ...
        public static async Task EnsureNotLastAdminAsync(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, Guid userId, CancellationToken ct)
        {
        ...
        public static Task<bool> IsAdminAsync(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, Guid userId, CancellationToken ct) =>
            unitOfWork.Repository<SysUserRole>().AnyAsync(ur => ur.UserId == userId && ur.Role.RoleType == ConstRole.AdminRoleType, ct);
    }
}

namespace ServerApiTemplate.Application.Features.V1.Users.Queries
{
    using ServerApiTemplate.Application.Features.V1.Users.DTOs;
    using ServerApiTemplate.Application.Features.V1.Users.Services;
    using ServerApiTemplate.Domain.Entities.Sys;
    ...
    public sealed class SearchUsersQueryHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork)
    ...
    public sealed class GetUserPermissionsQueryHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, IPermissionService permissionService)
...
namespace ServerApiTemplate.Application.Features.V1.Users.Commands
{
    ...
    public sealed class UpdateUserStatusCommandHandler(
        IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, ICurrentUser currentUser, IPermissionService permissionService, TimeProvider clock)
    ...
    {
        public AssignUserRolesCommandValidator(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork)
        {
    ...
    public sealed class AssignUserRolesCommandHandler(
        IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, ICurrentUser currentUser, IPermissionService permissionService)
    ...
    {
        public UpdateUserPermissionsCommandValidator(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork) =>
            RuleFor(x => x.Activities).NotNull().SetValidator(new ActivityPermissionInputsValidator(unitOfWork));
    }

    public sealed class UpdateUserPermissionsCommandHandler(IUnitOfWork<ServerApiTemplateDbContext> unitOfWork, IPermissionService permissionService)
