...
Backend .NET dựng từ **ServerApiTemplate** (Clean Architecture + CQRS + phân quyền 6 bảng).
...
- Handler inject `IUnitOfWork<ServerApiTemplateDbContext>`, không inject DbContext; `SaveChangesAsync` một lần ở cuối.
...
```bash
dotnet build ServerApiTemplate.sln
dotnet test tests/ServerApiTemplate.UnitTests
dotnet ef migrations add <Name> -p src/ServerApiTemplate.Persistence -s src/ServerApiTemplate.Persistence
dotnet run --project src/ServerApiTemplate.Api
```

Debug lỗi: lấy `traceId` trong ProblemDetails → `GET /api/v1/api-logs?traceId=...` → tìm `traceId` trong `src/ServerApiTemplate.Api/logs/*.log`.
