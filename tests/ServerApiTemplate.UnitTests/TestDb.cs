...
namespace ServerApiTemplate.UnitTests;
...
public sealed class TestDb : IDisposable
{
    public TestDb()
    {
        ...
        var options = new DbContextOptionsBuilder<ServerApiTemplateDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString())
            .AddInterceptors(new AuditSaveChangesInterceptor(audit, TimeProvider.System))
            .Options;
        Context = new ServerApiTemplateDbContext(options);
        UnitOfWork = new UnitOfWork<ServerApiTemplateDbContext>(Context, new ServiceCollection().BuildServiceProvider());
    }

    public ServerApiTemplateDbContext Context { get; }
    public IUnitOfWork<ServerApiTemplateDbContext> UnitOfWork { get; }
